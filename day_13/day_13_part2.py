...
class PacketPart:
    ...
    def __lt__(self, other):
        return True if compare_order(self, other) == 1 else False
...
def main():
    ...
    with open('input_day_13.txt') as file_input:
        for line in file_input:
            ...
            if line == '':
                continue
            packet_list.append(PacketPart(line))
    # Add the divider packets
    divider_1 = '[[2]]'
    divider_2 = '[[6]]'
    packet_list.append(PacketPart(divider_1))
    packet_list.append(PacketPart(divider_2))

    # sort the packets -> annoying as python does not support comparison with custom function
    packet_list.sort()

    debug('\nSorted: ------------------------------------------------------------------------')
    debug('\n'.join([str(x) for x in packet_list]))

    # Find index of divider packets
    packet_strings = [str(x) for x in packet_list]
    idx_1 = 1 + packet_strings.index(divider_1)
    idx_2 = 1 + packet_strings.index(divider_2)
    debug('Indexes: ' + str(idx_1) + ' ' + str(idx_2))

    decoder_key = idx_1 * idx_2
