...
class Monkey:
    ...
    def turn(self):
        ...
        while len(self.items) != 0:  # Must not loop over items in changing list
            ...
            # In part 2 this no longer works
            # # Relief
            # # - the relief causes the worry level to be divided by three and rounded down
            # item_worry_level = item_worry_level // 3

            # Modular arithmetics will save us from big numbers
            item_worry_level = item_worry_level % LEAST_COMMON_MULTIPLE
...
def main():
    ...
    # Set the LEAST_COMMON_MULTIPLE
    # https://stackoverflow.com/a/42472824
    from math import gcd
    a = [monke.test_divisible for monke in monkeys]  # will work for an int array of any length
    lcm = 1
    for i in a:
        lcm = lcm * i // gcd(lcm, i)
    ...
    for current_round in range(max_rounds):
        ...
        # # Debug part 2 - compare result with test data
        # if current_round + 1 in [1, 20, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000]:
        #     global DEBUG
        #     DEBUG = True
        #     debug('== After round ' + str(current_round + 1) + ' ==')
        #     for num, monkey in enumerate(monkeys):
        #         debug('Monkey ' + str(num) + ' inspected items ' + str(monkey.inspected_items) + ' times.')
        #     DEBUG = False
